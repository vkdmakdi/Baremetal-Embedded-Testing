...
int main(void)
{
  ...
  uint16_t dummy_data[8] = {0x1111, 0x2222, 0x3333, 0x4444, 0x5555, 0x6666, 0x7777, 0x8888};

  while (1)
  {
      Send_Packet_From_Buffer(&dummy_data[0]); // first 4
      Send_Packet_From_Buffer(&dummy_data[4]); // last 4
...
static void suwi(void)
{
    ...
    GPIOA->AFR[0] |=  (0x7U << (4 * 2)); // AF7 = USART2
